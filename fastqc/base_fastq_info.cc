...
namespace  gamtools {
    ...
    void BaseFqInfo::Add(const gamtools::BaseFqInfo &base_fq_info) {
        ...
        total_adapter_num += base_fq_info.total_adapter_num ;
        total_n_excess_num += base_fq_info.total_n_excess_num;
        total_low_qual_num += base_fq_info.total_low_qual_num;
    }
...
//        oss << head;
        if (fq_info2 != nullptr) { // PE
            oss << "Number of Reads:\t " << fq_info1->raw_read_num + fq_info2->raw_read_num <<
                "\t " << fq_info1->clean_read_num +  fq_info2->clean_read_num << std::endl;
            oss << "Data Size:\t " << fq_info1->raw_base_num + fq_info2->raw_base_num <<
                "\t " << fq_info1->clean_base_num + fq_info2->clean_base_num << std::endl;
            ...
            oss << "CG(%) of fq1:\t "  << std::setprecision(4) <<  (float)fq_info1->raw_base_gc * 100 / fq_info1->raw_base_num <<
                "\t "  <<   (float)fq_info1->clean_base_gc * 100 / fq_info1->clean_base_num << std::endl;
            oss << "CG(%) of fq2:\t "  << std::setprecision(4) << (float)fq_info2->raw_base_gc * 100 / fq_info2->raw_base_num <<
                "\t " <<   (float)fq_info2->clean_base_gc * 100  / fq_info2->clean_base_num << std::endl;
            oss << "Q20(%) of fq1:\t " << std::setprecision(4) << (float)fq_info1->raw_q20 * 100 / fq_info1->raw_base_num <<
                "\t " << (float)fq_info1->clean_q20 * 100 / fq_info1->clean_base_num << std::endl;
            oss << "Q20(%) of fq2:\t " << std::setprecision(4) << (float)fq_info2->raw_q20 * 100 / fq_info2->raw_base_num <<
                "\t " << (float)fq_info2->clean_q20 * 100 / fq_info2->clean_base_num << std::endl;
            oss << "Q30(%) of fq1:\t " << std::setprecision(4) << (float)fq_info1->raw_q30 * 100 / fq_info1->raw_base_num <<
                "\t " << (float)fq_info1->clean_q30 * 100 / fq_info1->clean_base_num << std::endl;
            oss << "Q30(%) of fq2:\t " << std::setprecision(4) << (float)fq_info2->raw_q30 * 100 / fq_info2->raw_base_num <<
                "\t " << (float)fq_info2->clean_q30 * 100 / fq_info2->clean_base_num << std::endl;
