...
void mem_aln2bam(const mem_opt_t *opt, const bntseq_t *bns, kstring_t *str, bseq1_t *s, int n, const mem_aln_t *list, int which, const mem_aln_t *m);
...
//            mem_aln2sam(opt, bns, &str, &s[0], n_aa[0], aa[0], i, &h[1]); // write read1 hits
            mem_aln2bam(opt, bns, &str, &s[0], n_aa[0], aa[0], i, &h[1]);
        }
//		s[0].sam = strdup(str.s); str.l = 0;
		s[0].bam_num = n_aa[0];
        s[0].bam = malloc(str.l);
        memcpy(s[0].bam, str.s, str.l);
        str.l = 0;

		for (i = 0; i < n_aa[1]; ++i) {
//            mem_aln2sam(opt, bns, &str, &s[1], n_aa[1], aa[1], i, &h[0]); // write read2 hits
            mem_aln2bam(opt, bns, &str, &s[1], n_aa[1], aa[1], i, &h[0]); // write read2 hits
