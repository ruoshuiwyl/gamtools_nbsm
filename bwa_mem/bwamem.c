...
static void bam_put_aux(const char *tag, const char type, const aux_value_t *value, kstring_t *str) {
	kputsn_(tag, 2, str);
...
static void put_bam_data(kstring_t *bam, const bseq1_t *s, const gam1_core_t *core, const kstring_t *bam_data ) {
    uint32_t  block_len = bam_data->l + 32;
    uint32_t x[8];
    x[0] = core->tid;
    x[1] = core->pos;
    x[2] = (uint32_t)core->bin<<16 | core->qual<<8 | core->l_qname;
    x[3] = (uint32_t)core->flag<<16 | core->n_cigar;
    x[4] = core->l_qseq;
    x[5] = core->mtid;
    x[6] = core->mpos;
    x[7] = core->isize;
    int is_rev = core->flag&0x10 ? 1 : 0;
...
void mem_aln2bam(const mem_opt_t *opt, const bntseq_t *bns, kstring_t *str, bseq1_t *s, int n, const mem_aln_t *list, int which, const mem_aln_t *m_)
{
	...
	core.l_qname = l_name;
	kputsn(s->name, l_name, &bam_data);

	core.flag = (p->flag&0xffff) | (p->flag&0x10000? 0x100 : 0);

	read_end_t read_end;
	memset(&read_end, 0, sizeof(read_end_t));
	...
	if (m && m->rid >= 0) {
		...
		if (p->rid == m->rid) {
			...
			if (m->n_cigar == 0 || p->n_cigar == 0) {
				...
				core.isize = -(p0 - p1 + (p0 > p1? 1 : p0 < p1? -1 : 0));
...
//			kputc('0', str);
	...
	if (!p->is_rev) {// the forward strand
		...
		core.l_qseq = qe - qb;
		int seq_len = (core.l_qseq + 1) >> 1;
		ks_resize(&bam_data, bam_data.l + seq_len);
...
//		kputc('\t', str);
		if (s->qual) { // printf qual
			ks_resize(&bam_data, bam_data.l + (qe - qb) + 1);
			for (i = qb; i < qe; ++i) {
				read_end.score += s->qual[i] >= 15 ? s->qual[i] : 0;
				bam_data.s[bam_data.l++] = s->qual[i];
			}
...
// print optional tags
	aux_value_t value;
	kstring_t temp_str = {0, 0, NULL};
	if (p->n_cigar) {
		value.i = p->NM;
		bam_put_aux("NM", 'i', &value, &bam_data);
		value.str = (char*)(p->cigar + p->n_cigar);
		bam_put_aux("MD", 'Z', &value, &bam_data);
...
//		kputsn("\tAS:i:", 6, str); kputw(p->score, str);
		value.i = p->score;
		bam_put_aux("AS", 'i', &value, &bam_data);
	}
	if (p->sub >= 0) {
		value.i = p->sub;
		bam_put_aux("XS", 'i', &value, &bam_data);
//		kputsn("\tXS:i:", 6, str); kputw(p->sub, str);
	}
	if (bwa_rg_id[0]) {
//		kputsn("\tRG:Z:", 6, str); kputs(bwa_rg_id, str);
		value.str = bwa_rg_id;
		bam_put_aux("RG", 'Z', &value, &bam_data);
	}
...
//			kputsn("\tSA:Z:", 6, str);
			for (i = 0; i < n; ++i) {
				...
				kputs(bns->anns[r->rid].name, &temp_str); kputc(',', &temp_str);
				kputl(r->pos+1, &temp_str); kputc(',', &temp_str);
				kputc("+-"[r->is_rev], &temp_str); kputc(',', &temp_str);
				for (k = 0; k < r->n_cigar; ++k) {
					kputw(r->cigar[k]>>4, &temp_str);
					kputc("MIDSH"[r->cigar[k]&0xf], &temp_str);
				}
				kputc(',', &temp_str); kputw(r->mapq, &temp_str);
				kputc(',', &temp_str); kputw(r->NM, &temp_str);
				kputc(';', &temp_str);
			}
			value.str = temp_str.s;
			bam_put_aux("SA", 'Z', &value, &bam_data);
			temp_str.l = 0;
		}
//		if (p->alt_sc > 0) {
...
//		}
	...
	if (p->XA) {
		value.str = p->XA;
		bam_put_aux("XA", 'Z', &value, &bam_data);
//		kputsn("\tXA:Z:", 6, str); kputs(p->XA, str);
	}
...
//		int tmp;
//		str->l = 0;
//		kputsn("\tXR:Z:", 6, &temp_str);
//		tmp = str->l;
//		kputs(bns->anns[p->rid].anno, &temp_str);
...
//		mem_aln2sam(opt, bns, &str, s, 1, &t, 0, m);
        mem_aln2bam(opt, bns, &str, s, 1, &t, 0, m);
...
//            mem_aln2sam(opt, bns, &str, s, aa.n, aa.a, k, m);
            mem_aln2bam(opt, bns, &str, s, aa.n, aa.a, k, m);
        }
